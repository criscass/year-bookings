...
<AppShell>
		...
		<AppBar gridColumns="grid-cols-3" slotDefault="place-self-center" slotTrail="place-content-end">
			...
			<svelte:fragment slot="trail">
				<button
					...
					class="btn btn-sm mr-12 px-4 text-lg {isEditable
						? 'variant-filled'
						: 'variant-filled-surface '}">{isEditable ? 'save' : '  +  '}</button
				>
				<LightSwitch />
			</svelte:fragment>
		</AppBar>
	...
	<!-- Page Route Content -->
	{#if isEditable}
		<div transition:slide={{ duration: 200 }} class="h-24 sticky top-0"><EditSlideDrawer /></div>
	{/if}
	<slot />
</AppShell>
